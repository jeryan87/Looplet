...
| intro | string | Opening paragraph text |
| outro | string | Closing paragraph text |
| show_intro | bool | Whether intro is included in the letter |
| show_outro | bool | Whether outro is included in the letter |
| share_token | string (unique, nullable) | UUID written at send time; powers the `view-letter` web view |
...
- [x] Improved email template design
- [x] Web view for recipients (nice browser rendering of letter)
- [ ] Draft auto-save
...
| 2026-05-24 | Improved email template + web view for recipients (Phase 2). Two changes shipped together: (1) Redesigned email HTML template (src/email/template.ts) — new visual improvements: green italic Georgia prompt labels replacing ALL CAPS; 3px green left accent bar per prompt section (table-based, Gmail/Outlook safe); smart photo grid with 3 explicit layouts (1 photo full-width hero, 2 photos equal columns, 3+ photos hero + strip); beige footer zone with hairline separator and subtle "Sent with Looplet" credit; header hairline separator; mobile-responsive <style> block for web view (email clients ignore it); optional "View in browser" preheader. (2) Web view for recipients — new Edge Function supabase/functions/view-letter/index.ts (public, JWT verification disabled): accepts ?token=UUID, uses service role key to fetch letter + loop + photos by share_token, returns full letter HTML as text/html. share_token column added to letters table (UUID unique, nullable, written at send time in send-letter). send-letter Edge Function updated to generate share_token via crypto.randomUUID(), write it to the DB before sending, and pass viewInBrowserUrl as 5th arg to buildEmailHtml so every sent email includes a "Having trouble reading this? View in browser" link. All three template copies (template.ts, send-letter, view-letter) synced — SYNC comment updated. schema.sql and CLAUDE.md updated to reflect intro/outro/show_intro/show_outro/share_token columns (previously missing from file despite existing in live DB). Deploy required: copy-paste view-letter/index.ts and send-letter/index.ts to Supabase dashboard; disable JWT verification on view-letter. |
| 2026-05-24 | Follow-up polish on email template after real-device testing. Changes: (1) Removed italic from intro/outro — now plain Georgia text matching the rest of the letter for consistency. (2) Removed divider lines above/below intro/outro — sections now breathe via spacing only. (3) Removed nested table approach for prompt sections (the 3px accent bar `<td>` was causing text indentation bug visible in email clients). Prompts now render as a simple single `<td>` with a small green sans-serif label (12px, 600 weight) above the response text. (4) Increased inter-prompt spacing 32px → 40px. (5) Header h1 bumped to 28px, bottom padding equalised to 32px both sides, date color slightly lighter. (6) view-letter Content-Type fix: changed from plain-object header `{ 'Content-Type': 'text/html; charset=utf-8' }` to `new Headers()` with lowercase `content-type: text/html` to prevent Supabase proxy from overriding the header — was causing browser to display raw HTML source instead of rendering. All three template copies synced (template.ts, send-letter, view-letter). Pending next session: per-prompt photo upload (requires prompt_id column on letter_photos + UI changes in WriteResponseScreen). Decision: photos must be user-assigned per prompt (not auto-distributed) because a user may attach multiple photos to one prompt. |
